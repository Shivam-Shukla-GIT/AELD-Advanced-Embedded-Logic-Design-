...
module AESL_axi_master_gmem (
    ...
    TRAN_gmem_BUSER,
    TRAN_gmem_Matrix_In,
    TRAN_gmem_Matrix_C_HW,
    ready,
    done
    );

//------------------------Parameter----------------------
`define TV_IN_gmem "../tv/cdatafile/c.matmul_MM_SP.autotvin_gmem.dat"
`define TV_OUT_gmem "../tv/rtldatafile/rtl.matmul_MM_SP.autotvout_gmem.dat"
...
output [32 - 1 : 0] TRAN_gmem_Matrix_In;
output [32 - 1 : 0] TRAN_gmem_Matrix_C_HW;
input ready;
input done;
...
reg RLAST_tmp = 0;
reg RVALID_tmp = 0;
reg [32 - 1 : 0] Matrix_In = 0;
reg [32 - 1 : 0] Matrix_C_HW = 0;
...
assign    TRAN_gmem_Matrix_In = Matrix_In;
assign    TRAN_gmem_Matrix_C_HW = Matrix_C_HW;
...
initial begin : initialize_offset
  ...
  Matrix_In <= 0 * DATA_byte_num;
  Matrix_C_HW <= 128 * DATA_byte_num;
end
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > gmem_mem_depth) begin
                $display ("D:/Sem_6/AELD/Labs/Lab_8_HLS/solution4/sim/verilog/AESL_axi_master_gmem.v: Write request address %d exceed AXI master gmem array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, gmem_mem_depth); 
                $finish;
            end
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > gmem_mem_depth) begin
                $display ("D:/Sem_6/AELD/Labs/Lab_8_HLS/solution4/sim/verilog/AESL_axi_master_gmem.v: Read request address %d exceed AXI master gmem array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, gmem_mem_depth); 
                $finish;
            end
